...
namespace Light_and_Shadow.Common
{
    public class TileDefinition : TagSerializable
    {
        ...
        public TileDefinition(short tileIndex, short wallIndex, Terraria.Tile tile, BitsByte extraDatas, BitsByte extraDatas2)
        {
            TileIndex = tileIndex;
            WallIndex = wallIndex;
        ...
        public static TileDefinition DeserializeData(TagCompound tag)
        {
            ...
            // 始终加载 ExtraDatas（如果存在）
            if (tag.ContainsKey("ExtraDatas"))
            {
